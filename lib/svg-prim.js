...
class SVG{
    constructor(chars, charsColor, shape, shapeColor, filename){
        ...
        if(!['circle', 'rect', 'polygon'].includes(shape)) throw new Error('shape must be a valid shape');
    ...
    getRawShape(){
        ...
        else if(this.shape === 'rect') return `<rect x="50" y="0" width="200" height="200" style="fill:${this.shapeColor}"/>`;
        else if(this.shape === 'polygon') return `<polygon points="0,200 300,200 150,0" style="fill:${this.shapeColor}"/>`;
    ...
    writeToFile(){
        // write to file using class filename
    }
}
var mySVG = new SVG('HH','#fefefe','circle','gray','logo');
console.log(mySVG.getRawSVG());
module.exports = SVG;